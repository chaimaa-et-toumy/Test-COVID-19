<?php
$path = ".";
$display ="d-none";
include '../includes/head.php';
include '../includes/nav-bar.php';
?>


    <main class=" resulte d-flex flex-column justify-content-center align-items-center gap-5 mt-3">
        <!-- steper -->
        <div class="d-flex justify-content-center w-100 my-4 ">
            <div class="col-md-4 col-sm-7 col-9">
                <div class="ques-line">
                    <span><i></i><h5>Information</h5></span>
                    <span><i></i><h5>Questionnaire</h5></span>
                    <span><i></i><h5>Resulte</h5></span>
                </div>
            </div>
        </div>
        <h2 class="pt-2 text-success">Lorem ipsum dolor sit amet consectetur.</h2>

        <div class="content card  px-5 py-4 rounded-3" style="width: 40%;">
                <h1 class=" text-center text-white" >d’urgences</h1>
                
                <p class="card-text text-center fs-4 text-warning">L’application est fournie à titre gratuit, en l’état,
                    uniquement à des fins d’information pour contribuer à fluidifier la prise en
                    charge des personnes par les services d’urgences pendant l’épidémie de Coronavirus
                    COVID-19. 
                </p>
            
        </div>
        <a href="introduction.php" class="ques-btn fw-bold fs-5 text-center text-decoration-none text-white" >recommence le test</a>
        
    </main>
    
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>